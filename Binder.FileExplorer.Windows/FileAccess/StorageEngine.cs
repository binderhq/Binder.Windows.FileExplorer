...
namespace Binder.Client.StorageEngine
{
	...
	public class StorageEngine
	{
		...
		/// <returns></returns>
		public void GetFile(string HiggsFileID, Stream stream, CancellationToken ct)
		{
			GetFile(HiggsFileID, l => { }, stream, ct);
		}
...
//						throw new EndpointNotFoundException("Resource could not be found.");
		...
		public void GetFile(string HiggsFileID, Action<long> transferProgress, Stream stream, CancellationToken ct)
		{
			Debug.Assert(HiggsFileID.Length == 64);
		...
		public StoreFileResponse StoreFile(Stream file, CancellationToken ct)
		{
			return StoreFile(file, l => { }, ct);
		}

		public StoreFileResponse StoreFile(Stream file, Action<long> transferProgress, CancellationToken ct)
		{
